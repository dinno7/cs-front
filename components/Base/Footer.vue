...
<template>
  <footer class="bg-gray-900" aria-labelledby="footer-heading">
    ...
    <div class="w-full px-6 pb-8  lg:px-8">
      ...
      <div
        class="mt-8 border-t border-white/10 pt-8 flex flex-col items-center md:flex-row md:items-center md:justify-around">
        <div class="flex gap-6 md:order-2">
          <a v-for="item in footer.social_media" :key="item.id" :href="item.url" target="_blank"
            class="text-gray-500 hover:text-gray-400">
            <span class="sr-only">{{ item.name }}</span>
            <component :is="socialMediaIcons[item.name].icon" class="h-6 w-6" aria-hidden="true" />
          </a>
        </div>
...
const footer = await find<{
  social_media: { name: string, url: string, id: number }[]
}>('footer', {
